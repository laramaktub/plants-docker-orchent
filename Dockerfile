...
RUN git clone https://github.com/laramaktub/cookie_plant_classification && \
    cd cookie_plant_classification && \
    pip install -e . && \
    cd ..
...
RUN curl -o ./cookie_plant_classification/plant_classification/training_weights/${THEANO_TR_WEIGHTS} \
    ${SWIFT_CONTAINER}${THEANO_TR_WEIGHTS}

RUN curl -o ./cookie_plant_classification/plant_classification/training_info/${THEANO_TR_JSON} \
    ${SWIFT_CONTAINER}${THEANO_TR_JSON}

RUN curl -o ./cookie_plant_classification/data/data_splits/synsets.txt \
